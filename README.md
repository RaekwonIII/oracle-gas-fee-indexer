...
A subgraph that indexes the **`WeightedRootProposed`** event of the SSVNetwork
...
Each `WeightedRootProposed` event marks a **successful oracle root submission**
(the event is only emitted when the transaction succeeds), so every emitted event
is counted. `ClusterBalanceUpdated` and `RootCommitted` are **not** indexed.

For every event the handler reads the **transaction receipt** (enabled via
...
| Entity | Purpose |
| --- | --- |
| `GasFeePayment` | `@entity(timeseries: true)` — one immutable data point per indexed event (sender, gasFee, gasUsed, gasPrice, txHash). `id`/`timestamp` are auto-managed by graph-node. |
